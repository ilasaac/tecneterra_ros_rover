...
timeout "$TIMEOUT" docker exec agri_rover_rv1 bash -c \
  'source /opt/ros/*/setup.bash && source /workspaces/isaac_ros-dev/install/setup.bash && ros2 run agri_rover_simulator sim_harness --ros-args -r __ns:=/rv1 -p use_sim_time:=false --params-file /workspaces/isaac_ros-dev/install/agri_rover_bringup/share/agri_rover_bringup/config/rover1_params.yaml' \
  2>&1
