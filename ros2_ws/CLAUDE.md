...
- **Differential-drive physics** (`sim_navigator.py`, `DiffDriveState`): the SIL simulator always uses a physics model ported from TTR Robot.cpp, regardless of control algorithm. Key properties:
  ...
  - `--algo stanley/mpc/ttr --track-width 0.9` CLI overrides in `sim_navigator.py`
